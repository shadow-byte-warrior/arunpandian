...
import { Database, Sigma, BarChart3, Wrench, type LucideIcon } from 'lucide-react';
...
const iconMap: Record<string, LucideIcon> = { 
  ...
  "Data Analysis & Wrangling": Wrench,
  "Languages & Querying": Database
};
...
export default function Skills({ data }: SkillsProps) {
  const ticker = ['Pandas', 'NumPy', 'Power BI', 'PostgreSQL', 'Scikit-learn', 'TensorFlow', 'DAX'];

  return (
    <section id="skills" className="py-24 sm:py-32 bg-bg">
      <div className="max-w-7xl mx-auto px-5 sm:px-8">
        <div className="mb-14">
          <span className="text-xs font-mono tracking-[0.25em] text-accent uppercase">02 — Expertise</span>
          <h2 className="mt-3 font-display font-extrabold text-3xl sm:text-5xl text-ink tracking-tight">Capabilities</h2>
        </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-5">
          {Object.entries(data).map(([category, items], i) => {
            ...
            return (
              ...
              >
                <div className="h-11 w-11 grid place-items-center rounded-xl bg-slate-100 text-ink group-hover:bg-ink group-hover:text-white transition-colors">
                  <Icon size={20} />
                </div>
                <h3 className="mt-5 font-display font-bold text-lg text-ink">{category}</h3>
