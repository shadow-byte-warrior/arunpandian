...
export default function Hero({ data }: HeroProps) {
  const story = [
    { k: '01', label: 'Wrangling' },
    { k: '02', label: 'EDA' },
    { k: '03', label: 'Automation' },
    { k: '04', label: 'Decisions' }
  ];
  
  const ticker = ['SQL', 'Python', 'Power BI', 'Data Analytics', 'Automation'];
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8 grid lg:grid-cols-12 gap-10 lg:gap-8 items-center">
        <div className="lg:col-span-7">
          ...
          <h1 className="mt-6 font-display font-extrabold text-ink leading-[0.92] tracking-tight text-[clamp(2.75rem,7vw,5.5rem)]">
            <motion.span custom={1} variants={rise} initial="hidden" animate="show" className="block">Turning messy data</motion.span>
            <motion.span custom={2} variants={rise} initial="hidden" animate="show" className="block">into clear decisions</motion.span>
            <motion.span custom={3} variants={rise} initial="hidden" animate="show" className="block">
              <span className="italic font-medium text-accent">with AI & Analytics.</span>
            </motion.span>
          </h1>
          ...
          >
            ...
            <MagneticButton
              href="/resume.pdf" download strength={0.25}
          ...
          >
            <div className="flex items-center gap-5 text-ink-soft">
              <a href={data.github} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="hover:text-ink transition-colors"><GithubIcon /></a>
              <a href={data.linkedin} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="hover:text-accent transition-colors"><LinkedinIcon /></a>
              <a href={`mailto:${data.email}`} aria-label="Email" className="hover:text-rose-500 transition-colors"><Mail size={20} /></a>
            </div>
          </motion.div>
        </div>
            ...
            >
              ...
              <div className="relative rounded-[1.75rem] border border-line bg-surface p-2 shadow-[0_40px_90px_-30px_rgba(9,9,11,0.4)]">
                ...
                <div className="absolute bottom-5 left-5 right-5 flex items-center justify-between rounded-xl bg-surface/85 backdrop-blur px-4 py-2.5 border border-line">
                  <span className="font-display font-semibold text-ink text-sm">Data · in motion</span>
                  <span className="text-xs text-ink-soft font-mono">SQL · Python · BI</span>
                </div>
              </div>
              ...
              <div className="absolute -top-5 -left-5 sm:-left-8 flex items-center gap-3 rounded-2xl bg-surface/90 backdrop-blur border border-line shadow-[0_20px_45px_-20px_rgba(9,9,11,0.5)] pl-2 pr-4 py-2">
                ...
                <div className="leading-tight">
                  <p className="font-display font-bold text-ink text-sm">{data.name}</p>
                  <p className="text-[11px] text-ink-soft font-mono">{data.title}</p>
                </div>
              </div>
            </motion.div>
          </motion.div>
