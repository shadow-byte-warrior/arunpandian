...
export default function Contact() {
  ...
  const [status, setStatus] = useState({ submitting: false, success: false as boolean | null, message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setStatus({ submitting: false, success: true, message: 'Message sent successfully! (Simulated)' });
    ...
    <section id="contact" className="py-24 sm:py-32 bg-bg">
      <div className="max-w-6xl mx-auto px-5 sm:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          >
            <span className="text-xs font-mono tracking-[0.25em] text-accent uppercase">05 — Contact</span>
            ...
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-1 text-sm text-ink-soft">
              <a href={`tel:${resumeData.personalInfo.phone}`} className="hover:text-ink transition-colors">{resumeData.personalInfo.phone}</a>
          ...
          >
            ...
            <MagneticButton
              strength={0.2}
              className="w-full gap-2 rounded-xl bg-ink text-white py-3.5 text-sm font-semibold hover:bg-accent disabled:opacity-50 transition-colors inline-flex justify-center items-center"
            >
              {status.submitting ? 'Sending…' : 'Send message'}
              <Send size={15} className={status.submitting ? 'animate-pulse ml-2' : 'ml-2'} />
            </MagneticButton>
          </motion.form>
        </div>
      </div>
