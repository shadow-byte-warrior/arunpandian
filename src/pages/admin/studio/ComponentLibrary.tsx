...
const NAVBAR_THUMBS = [
  ...
  { value: 'capsule', label: 'Capsule', preview: <div className="flex items-center justify-center h-full"><div className="flex items-center gap-2 bg-white border border-[#ddd] rounded-full px-3 py-1 shadow-sm">{['•','—','•'].map((l,i)=><span key={i} className="h-1 w-4 bg-[#bbb] rounded" />)}</div></div> },
  { value: 'minimal', label: 'Minimal', preview: <div className="flex items-center gap-2 h-6 px-2"><span className="h-2 w-8 bg-[#111] rounded" /><span className="flex-1" />{['/','/'].map((l,i)=><span key={i} className="h-1.5 w-6 bg-[#aaa] rounded" />)}</div> },
  { value: 'karolbinkowski', label: 'Brutalist', preview: <div className="flex items-center h-full border-b-[3px] border-[#111] bg-[#0f0f0f] px-2 gap-2"><span className="h-3 w-3 bg-[#e0fd60] rounded-sm" /><span className="flex-1" />{['',''].map((l,i)=><span key={i} className="h-1.5 w-6 bg-[#555] rounded" />)}</div> },
  { value: 'pelizzari', label: 'Transparent', preview: <div className="flex items-center h-full px-2 gap-2 bg-transparent"><span className="h-2 w-6 bg-[#d63e2e] rounded" /><span className="flex-1" />{['','',''].map((l,i)=><span key={i} className="h-1.5 w-5 bg-[#888] rounded" />)}</div> },
  { value: 'vividmotion', label: 'Glass', preview: <div className="flex items-center h-full px-2 gap-2 bg-white/20 backdrop-blur rounded-lg border border-white/20"><span className="h-2 w-6 bg-[#111]/50 rounded" /><span className="flex-1" />{['',''].map((l,i)=><span key={i} className="h-1.5 w-5 bg-[#888] rounded" />)}</div> },
